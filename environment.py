...
class Environment:
    ...
    def __init__(self, display, magnification, id=0):
        ...
        self.init_state = np.array([0.05, 0.05], dtype=np.float32)
        # Set the initial state of the goal
        self.goal_state = np.array([0.95, 0.95], dtype=np.float32)
        # Set the space which the obstacle occupies
        self.obstacle_space = np.array([[0.3, 0.5], [0.3, 0.6]], dtype=np.float32)
        self.obstacles = np.array([[0.35, 0.35], [0.35, 0.45], [0.35, 0.55],
                                [0.45, 0.35], [0.45, 0.45], [0.45, 0.55]], dtype=np.float32)
        ...
        self.pos = np.stack((self.pos[0], self.pos[1])).astype('float32')
        self.states = torch.tensor([self.pos[:,i,j] for i in range(10) for j in range(10) \
            if not (np.abs(self.pos[:,i,j] - np.concatenate((self.obstacles, self.goal_state.reshape(1,-1)), 0))< 0.0001).all(1).any() ]).float()
    ...
    # Function to plot the Q functions
    def plot_Qs(self, Q):
    ...
    # Function to plot a trace of states
    ...
    def reset(self):
        # state = np.array(random.choices(self.states)[0], dtype='float32')
        # return state
